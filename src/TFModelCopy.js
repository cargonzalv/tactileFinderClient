...
export class TFModel {
  ...
  async loadModel(img) {
    ...
    // quick.
    tf.tidy(() => this.mobilenet.predict(this.image.capture(img)));
  ...
  async train() {
    ...
    console.log(controllerDataset.xs)
    ...
    const batchSize =
      Math.floor(controllerDataset.xs.shape[0]  );
    if (!(batchSize > 0)) {
      throw new Error(
    ...
    this.model.fit(controllerDataset.xs, controllerDataset.ys, {
      ...
      callbacks: {
        ...
        onEpochEnd: async (epoch, log) => {
          if(epoch == 19){
            console.log(`finished with loss = ${log.loss}`)
            const saveResult = await this.model.save('http://model-server.domain/upload');
